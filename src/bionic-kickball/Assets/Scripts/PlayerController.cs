<<<<<<< HEAD:src/bionic-kickball/Assets/Scripts/PlayerController.cs
﻿using UnityEngine;
...
public class PlayerController: MonoBehaviour
{
	...
	void Update()
	{
		...
		// Gather power to kick the ball w/ space
		if (Input.GetKey(KeyCode.K) && kickPower < kickPowerLimit) {
			kickPower += Time.deltaTime;
			Debug.Log("increasing kick power: " + kickPower);
		} 
		if (Input.GetKeyUp(KeyCode.K)) {
			// if released space, check to make sure ball is close enough to the player
			playerToBallDistance = Vector3.Distance(ball.transform.position, this.transform.position);
			ballBelowPlayer = ball.transform.position.y < this.transform.position.y;
			if (kickPower > 0  && playerToBallDistance < acceptablePToBDistance) {
				KickBall();
				print("kicked ball");
			}
...
>>>>>>> a85f929883eae7291282bb2773ebd63d0510c973:src/bionic-kickball/Assets/Sprites/Scripts/PlayerController.cs
}